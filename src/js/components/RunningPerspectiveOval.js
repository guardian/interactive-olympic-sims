...
export default function RunningPerspectiveOval(data,options) {
	...
	function buildVisual() {
		...
		xscale=scaleLinear().domain([0,dimensions.radius*2+dimensions.field.width]).rangeRound([0,WIDTH]),
		yscale=scaleLinear().domain([0,dimensions.lanes*2+dimensions.field.height]).rangeRound([0,HEIGHT]);
		...
		leg.append("path")
				...
				.attr("stroke-width",yscale(dimensions.lane-dimensions.line_width*2)*0.9)
				/*.attr("d",s=>{
				...
				.selectAll("text")
					...
					.append("textPath")
				    	...
				    	.attr("startOffset",s=>{
				    		...
				    		let d=dimensions.lane_staggers[s.lane]/400;
				    		
				    		console.log(s.lane,dimensions.lane_staggers,dimensions,(50 - d*100) + "%")
				    		if(s.lane===7) {
				    			return "44%";
				    		}
				    		return (50 - d*100 + (s.lane-1)*0.1) + "%";
				    	})
				    	.text((s,i)=>{
				...
				.selectAll("text")
				    ...
				    .attr("dy",s=>{
				  	...
				  	.append("textPath")
				    	...
				    	.attr("text-anchor",s=>(s.distance>0)?"end":"start")
		...
		selected_leg
				...
				.selectAll("textPath")
				    	.attr("startOffset",s=>{
				    		...
				    		return (((s.mt-delta) / dimensions.length)*100)+"%";
		...
		selected_leg
				.select("path")
					.attr("stroke-dasharray", function(s){
						...
						let t = s.distance>0?((s.mt-delta)/dimensions.length):0;
